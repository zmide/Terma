...
> This refreshed v1.5.2 build adds pagination to forwarding lists and replaces the README captures with verified English and Simplified Chinese interface screenshots. If you installed the earlier v1.5.2 build, open Terma's update page and choose Download again.
...
- README screenshots are now generated separately for English and Simplified Chinese, with language-matched demo data and automated checks for mixed-language UI and sensitive placeholders.
...
> 此次刷新 v1.5.2 构建，为转发列表加入分页，并将 README 截图替换为已验证的英文和简体中文界面截图。如果已安装较早的 v1.5.2，请在 Terma 更新页面选择“重新下载”。
...
- README 现在分别使用英文和简体中文截图，并通过语言混用和敏感占位内容检查。
