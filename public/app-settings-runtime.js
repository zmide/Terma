...
function normalizeAiSettingsResponse(value={}) {
  ...
  return {
    ...
    reasoning_effort:["none", "minimal", "low", "medium", "high", "xhigh", "max"].includes(String(source.reasoning_effort || "").toLowerCase()) ? String(source.reasoning_effort).toLowerCase() : "none",
...
function aiSettingsPanelHtmlLegacy() {
  ...
  return `<section class="terminal-ai-settings">
    ...
    <div class="grid terminal-ai-settings-grid terminal-ai-reasoning-settings">
      <div><label for="terminalAiReasoningEffort">${esc(tr("settings:ai.reasoning_effort", {defaultValue:"推理强度"}))}</label><select id="terminalAiReasoningEffort"><option value="none" ${ai.reasoning_effort === "none" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_none", {defaultValue:"关闭"}))}</option><option value="minimal" ${ai.reasoning_effort === "minimal" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_minimal", {defaultValue:"最低"}))}</option><option value="low" ${ai.reasoning_effort === "low" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_low", {defaultValue:"低"}))}</option><option value="medium" ${ai.reasoning_effort === "medium" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_medium", {defaultValue:"中"}))}</option><option value="high" ${ai.reasoning_effort === "high" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_high", {defaultValue:"高"}))}</option><option value="xhigh" ${ai.reasoning_effort === "xhigh" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_xhigh", {defaultValue:"极高"}))}</option><option value="max" ${ai.reasoning_effort === "max" ? "selected" : ""}>${esc(tr("settings:ai.reasoning_max", {defaultValue:"最大"}))}</option></select><small class="muted">${esc(tr("settings:ai.reasoning_current", {effort:ai.reasoning_effort, defaultValue:`当前：${ai.reasoning_effort}`}))}</small></div>
      <div><label class="check-row"><input id="terminalAiDeepThinking" type="checkbox" ${ai.deep_thinking ? "checked" : ""}> ${esc(tr("settings:ai.deep_thinking", {defaultValue:"深度思考"}))}</label><small class="muted">${esc(tr("settings:ai.deep_thinking_hint", {defaultValue:"开启后未选择推理强度时会使用高；已选择极高或最大时不会被降级。是否生效取决于服务商；界面只显示服务商返回的推理摘要，不显示隐藏思维链。"}))}</small></div>
    </div>
