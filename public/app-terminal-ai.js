...
function terminalAiStateForKey(key) {
  ...
  if (!terminalAiStates.has(normalized)) terminalAiStates.set(normalized, {
    ...
    modelCustom:false,
    reasoning_effort:["none", "low", "medium", "high"].includes(String(terminalAiSettingsValue().reasoning_effort || "").toLowerCase()) ? String(terminalAiSettingsValue().reasoning_effort).toLowerCase() : "none",
  ...
  if (!["none", "low", "medium", "high"].includes(String(state.reasoning_effort || "").toLowerCase())) state.reasoning_effort = "none";
...
function terminalAiMovePointer(event) {
  ...
  if (active.mode === "drag") {
    ...
    // A floating panel may be resized flush to the viewport bottom. Dragging
    // itself keeps an 8px safety margin, but the resize handle should not leave
    // an unreachable strip at the bottom edge.
...
function terminalAiPanelHtml(key) {
  const state = terminalAiStateForKey(key);
  const layout = terminalAiLayoutSettings();
  return `<section class="terminal-ai-panel${state.open ? " is-open" : ""}" data-terminal-ai-key="${escAttr(key)}"${state.open ? "" : " hidden"}>
    ...
    <div class="terminal-ai-body" data-terminal-ai-body="${escAttr(key)}">
      <details class="terminal-ai-blocks-wrap"><summary><span>${icon("square-terminal")}<b>${esc(tr("terminal:ai.blocks", {defaultValue:"命令记录"}))}</b></span><span class="terminal-ai-block-count" data-terminal-ai-count="${escAttr(key)}">${state.blocks.length}</span></summary><div class="terminal-ai-blocks" data-terminal-ai-blocks="${escAttr(key)}"></div></details>
      <div class="terminal-ai-turns" data-terminal-ai-turns="${escAttr(key)}"></div>
      <div class="terminal-ai-approval-slot" data-terminal-ai-approval="${escAttr(key)}"></div>
      ...
      <form class="terminal-ai-composer" data-submit-action="terminal-ai-submit" data-terminal-ai-key="${escAttr(key)}">
        <textarea data-terminal-ai-prompt="${escAttr(key)}" data-keydown-action="terminal-ai-prompt-keydown" data-terminal-ai-key="${escAttr(key)}" rows="3" maxlength="6000" placeholder="${escAttr(tr("terminal:ai.prompt_placeholder", {defaultValue:"描述任务，或让 AI 检查、排错并给出下一步…"}))}"></textarea>
...
function renderTerminalAiPanel(key) {
  ...
  if (blocks) blocks.innerHTML = state.blocks.length ? state.blocks.slice().reverse().map(block => terminalAiBlockHtml(key, block, state.selectedBlockId)).join("") : `<div class="terminal-ai-empty">${esc(tr("terminal:ai.empty_blocks", {defaultValue:"执行命令后，这里会显示命令和已捕获的输出"}))}</div>`;
  ...
  if (approval) approval.innerHTML = state.pendingApproval
    ? terminalAiApprovalHtml(key, state.pendingApproval)
    : state.agentWaitingForInput
      ? "<div class=\"terminal-ai-waiting-input\" role=\"alert\">" + icon("terminal-square") + "<div><strong>" + esc(tr("terminal:ai.waiting_input_title", {defaultValue:"终端正在等待人工输入"})) + "</strong><span>" + esc(tr("terminal:ai.waiting_input_hint", {defaultValue:"请在终端中输入 yes、no 或完整主机指纹。Terma 已暂停 Agent，不会继续发送命令。"})) + "</span><code>" + esc(state.agentWaitingForInput.command || "") + "</code></div></div>"
      : "";
  ...
  if (turns) {
    ...
    turns.innerHTML = state.error
      ...
      : state.turns.length
        ...
        : `<div class="terminal-ai-welcome">${icon("message-square-text")}<span>${esc(tr("terminal:ai.welcome", {defaultValue:"直接描述任务。Agent 可以结合已选终端内容或命令输出，逐步给出检查和修复操作。"}))}</span></div>`;
  ...
  if (prompt) prompt.disabled = state.busy;
...
function finalizeTerminalAiActiveBlock(session) {
  ...
  if (terminalAiBlockNeedsInput(block)) {
    ...
    state.agentWaitingForInput = state.agentWaitingForInput || {blockId:block.id, command:block.command};
...
function terminalAiBlockNeedsInput(block) {
  ...
  return /(?:\[y\/n\]|\[Y\/N\]|continue\?|press\s+(?:enter|return)|password\s*:?|please\s+type\s+['\"]?yes['\"]?\s*,?\s+['\"]?no['\"]?\s+or\s+the\s+fingerprint|type\s+['\"]?yes['\"]?\s*,?\s+['\"]?no['\"]?\s+or\s+the\s+fingerprint|are\s+you\s+sure\s+you\s+want\s+to\s+continue\s+connecting\s*\(yes\/no(?:\/\[fingerprint\])?\))\s*:?\s*$/im.test(output);
}

function captureTerminalAiBlockCommand(session, command) {
  if (!terminalAiConfigured() || !session || session.sensitiveInput || !String(command || "").trim()) return;
  finalizeTerminalAiActiveBlock(session);
  const state = terminalAiStateForKey(session.key);
  const block = {id:createTerminalLogId(), command:String(command).trim().slice(0, 2000), output:"", startedAt:Date.now(), completedAt:0};
...
function terminalAiHistory(state, currentTurnId, currentTaskId="") {
  ...
  return terminalAiTaskGroups(previous).slice(-6).flatMap(group => {
    const answer = terminalAiDeduplicateRepeatedText([...group.steps].reverse().find(turn => turn.answer && !turn.error)?.answer || "");
...
async function streamTerminalAiTurn(key, prompt, contexts, options={}) {
  ...
  try {
    ...
    const result = await request({message:requestPrompt, contexts, history:Array.isArray(options.history) ? options.history : terminalAiHistory(state, turn.id, turn.taskId), locale:normalizeTermaLanguage(document.documentElement.lang), permission:state.permission, ai:{model:state.model || terminalAiSettingsValue().model || "", reasoning_effort:state.reasoning_effort, deep_thinking:state.deep_thinking}}, {
...
async function submitTerminalAiRequest(key, prompt, contexts, options={}) {
  ...
  const taskId = createTerminalLogId();
  const controller = new AbortController();
  state.controller = controller;
  renderTerminalAiPanel(key);
  let currentPrompt = originalPrompt;
  let currentContexts = Array.isArray(contexts) ? contexts : [];
  let mcpContexts = [];
  ...
  try {
      ...
      if (explicitSearch) {
        ...
        if (state.permission !== "suggest") {
          ...
          if (searchCall) {
            ...
            const resultEntry = {kind:"mcp", server:searchCall.server, tool:searchCall.tool, arguments:searchCall.arguments || {}, output:"", status:tr("terminal:ai.agent_not_executed", {defaultValue:"未执行"})};
            try {
              const execution = await executeTerminalAiMcpCall(key, searchCall, {index:1, total:1});
              resultEntry.output = execution.output || "";
              resultEntry.status = execution.unsupported
                ? tr("terminal:ai.mcp_unsupported", {tool:searchCall.tool, defaultValue:`未执行：未发现或未启用工具 ${searchCall.tool}`})
                : execution.called
    ...
    while (!controller.signal.aborted && state.requestId === requestId) {
      if (options.agentTask !== true) {
        const turn = await streamTerminalAiTurn(key, currentPrompt, currentContexts, {requestId, taskId, controller, agentContinuation:continuation, streamRequest:options.streamRequest});
        ...
        for (const command of (Array.isArray(commands) ? commands : [])) {
          ...
          commandRunner = commandRunner.then(async () => {
            ...
            try {
              ...
              const risk = terminalAiCommandRisk(normalized, {userPrompt:originalPrompt});
              ...
              const execution = await executeCommand(key, normalized, {auto:automatic, details:true, risk, userPrompt:originalPrompt, index, total:Math.max(index, state.agentQueue.length)});
              if (!execution?.sent) {
                resultEntry.status = execution?.denied ? tr("terminal:ai.agent_denied", {defaultValue:"用户拒绝执行"}) : tr("terminal:ai.agent_not_executed", {defaultValue:"未执行"});
                ...
                if (completed) {
                  const status = completed.waitingForInput
                    ...
                    : completed.timedOutAt
                      ? tr("terminal:ai.timed_out", {defaultValue:"等待命令完成超时，已交回 Agent 分析"})
                      : completed.exitCode && completed.exitCode !== 0
                        ...
                        : tr("terminal:ai.completed", {defaultValue:"已完成"});
                  resultEntry = {command:execution.command, output:completed.output || "", status, waitingForInput:Boolean(completed.waitingForInput)};
                  if (completed.waitingForInput) state.agentWaitingForInput = {taskId, blockId:completed.id, command:execution.command, originalPrompt, contexts:currentContexts};
                }
              }
            } catch (error) {
              resultEntry.status = String(error?.message || tr("terminal:ai.agent_not_executed", {defaultValue:"未执行"}));
      ...
      const turn = await streamTerminalAiTurn(key, currentPrompt, currentContexts, {requestId, taskId, controller, agentContinuation:continuation, displayPrompt:continuation ? undefined : originalPrompt, history:explicitSearch ? [] : undefined, streamRequest:options.streamRequest, onActions:enqueueActions});
      ...
      if (results.some(item => item.waitingForInput)) break;
      if (!results.length) {
        ...
        // A compatible model can need more than one planning turn before it
        // emits the next action. Keep the task alive and ask for either one
        // concrete action or a final conclusion; only an explicit final answer
        // or user cancellation ends the loop.
        if (awaitingActionRecovery && terminalAiAnswerNeedsActionRecovery(turn?.answer || "")) {
          currentPrompt = terminalAiAgentActionRecoveryPrompt(originalPrompt, turn.answer);
          currentContexts = [...mcpContexts];
          continuation = true;
...
async function submitTerminalAiPrompt(key) {
  ...
  return submitTerminalAiRequest(key, prompt, terminalAiContextForKey(key), {agentTask:true});
}
...
async function executeTerminalAiCommand(key, command, options={}) {
  ...
  const sent = sendTerminalData(key, `${value}\r`, {trackCommand:true, focus:false});
  ...
  return options.details === true ? {sent, blockId:block?.id || session.aiActiveBlockId || "", command:value, risk} : sent;
}
...
function waitForTerminalAiCommandCompletion(key, blockId, options={}) {
  ...
  return new Promise(resolve => {
    ...
    const check = () => {
      ...
      if (block && Date.now() - startedAt >= TERMINAL_AI_COMMAND_WAIT_MS) {
