...
let linuxDesktopManagerPollTimer = null;
...
async function openLinuxDesktopTask(connectionId, taskId) {
  ...
  try {
    ...
    if (task.status === "running") pollLinuxDesktopInstallTask(task.id);
  } catch (error) {
...
const REMOTE_INSTALL_MODE_META = {
  ...
  "local-offline":{label:"本机下载后离线安装", icon:"hard-drive-download", description:"仅支持 Debian/Ubuntu 及兼容 APT/.deb 系统；TunnelDesk 会在本机下载依赖并通过 SFTP 上传安装。"},
...
function remoteInstallModesMarkup(plan, runExpression, manualExpression="revealRemoteInstallManual(this)") {
...
function remoteInstallManualMarkup(plan, options={}) {
  ...
  const manualOfflineStep = commands.length
    ? localPackages.length
      ? `如果远端和运行 TunnelDesk 的本机都无法联网，请在另一台与远端发行版、版本及 CPU 架构匹配的联网机器下载 ${localPackages.join("、")} 及全部依赖，再通过本地文件/SFTP 上传后按发行版的离线安装方式执行。`
      : "如果远端和运行 TunnelDesk 的本机都无法联网，请在另一台与远端发行版、版本及 CPU 架构匹配的联网机器下载所需组件和全部依赖，再通过本地文件/SFTP 上传后按发行版的离线安装方式执行。"
    : "";
...
function watchRemoteComponentTask(task, options={}) {
  ...
  const watcher = {cancelled:false, timer:null};
  remoteComponentTaskWatchers.set(id, watcher);
  const scopeMatches = () => remoteComponentTaskScopeMatches(options.scope);
  const resolveContainer = () => {
    const container = typeof options.container === "function" ? options.container() : options.container;
    return remoteComponentTaskContainer(options.scope, container);
  };
  const tick = async current => {
    if (watcher.cancelled) return;
    const container = resolveContainer();
    if (container) renderRemoteComponentTask(container, current, options);
    if (typeof refreshSftpJobs === "function") void refreshSftpJobs();
    const status = String(current?.status || "").toLowerCase();
    if (["done", "failed", "cancelled"].includes(status)) {
      remoteComponentTaskWatchers.delete(id);
      const completionContainer = container || resolveContainer();
      if (scopeMatches() && status === "done" && typeof options.onDone === "function") await options.onDone(current, completionContainer);
      if (scopeMatches() && status === "failed" && typeof options.onFailed === "function") await options.onFailed(current, completionContainer);
      return;
    }
    watcher.timer = setTimeout(async () => {
      try {
        ...
        const container = resolveContainer();
        if (container) renderRemoteComponentTask(container, {...current, status:"failed", error:error.message || "远程组件任务状态读取失败"}, options);
        remoteComponentTaskWatchers.delete(id);
      }
    }, 1200);
  };
...
async function requestRemoteAdminAuthorization(connectionId, scope="远端管理") {
  const connection = currentConnection(Number(connectionId || 0));
  ...
  return new Promise(resolve => {
    ...
    modal.innerHTML = `<form class="modal-card remote-admin-modal" role="dialog" aria-modal="true" aria-labelledby="remoteAdminTitle">
      ...
      <div class="connection-test-status warning">仅用于这一次远端管理操作。账号、密码、私钥口令不会保存到连接、配置、日志或任务中心。</div>
    ...
    const syncSudoMode = () => {
      ...
      if (sameSudoOption.disabled && sudoMode.value === "same") sudoMode.value = "separate";
    ...
    onSubmit = event => {
      ...
      finish({ssh_user:user, auth_method:authMethod, ssh_password:authMethod === "password" ? sshPassword : "", identity_file:authMethod === "key" ? key : "", private_key_passphrase:authMethod === "key" ? passphrase : "", sudo_requested:true, sudo_password:sudoPassword});
    };
...
function renderSshX11ForwardingPanel(connection, sshX11, source="xserver") {
  ...
  const automaticAction = sshX11.can_manage
    ? `<button class="${action === "enable" ? "primary" : "danger"}" type="button" onclick="changeSshX11Forwarding('${action}',this,${Number(connection.id)},'${sourceArg}')">${icon(action === "enable" ? "shield-check" : "shield-off")}<span>${action === "enable" ? "开启 X11 转发" : "关闭 X11 转发"}</span></button>`
    : `<button class="${action === "enable" ? "primary" : "danger"}" type="button" onclick="changeSshX11Forwarding('${action}',this,${Number(connection.id)},'${sourceArg}')">${icon("key-round")}<span>临时授权后${action === "enable" ? "开启" : "关闭"}</span></button>${sshX11.can_terminal_manage && sshX11.terminal_commands?.[action] ? `<button type="button" onclick="openSshX11ConfigureTerminal(${Number(connection.id)},'${action}','${sourceArg}')">${icon("square-terminal")}<span>在终端手动${action === "enable" ? "开启" : "关闭"}</span></button>` : ""}`;
...
function openLinuxDesktopManager(connectionId=0, updateTab=true) {
  clearInterval(linuxDesktopManagerPollTimer);
  linuxDesktopManagerPollTimer = null;
  const normalizedConnectionId = Number(connectionId || 0);
  linuxDesktopManagerState = {connectionId:normalizedConnectionId, diagnostics:null, sshX11:null, taskId:"", task:null, logs:[]};
  ...
  renderLinuxDesktopManager();
  if (linuxDesktopManagerState.connectionId) loadLinuxDesktopManager();
}
...
function renderLinuxDesktopUsageBadges(diagnostics, desktopId) {
  ...
  if (usage.vnc_configured) add("protocol", "VNC 配置", "TigerVNC/Xvnc 启动配置已明确选择此桌面，不代表当前会话正在运行");
  ...
  if (usage.vnc_virtual_active) add("active", "VNC 虚拟会话", "检测到 TigerVNC/Xvnc 虚拟会话正在使用此桌面");
...
function renderLinuxDesktopManager() {
  ...
  view.innerHTML = `<div class="linux-desktop-manager remote-desktop-launch">
    ...
    ${selected && diagnostics ? `<div class="linux-desktop-diagnostics ${macos ? state.sshX11?.ready ? "ready" : "warning" : diagnostics.has_desktop ? "ready" : "warning"}"><div><strong>${esc(diagnostics.connection?.name || "SSH 主机")}</strong><span>${macos ? "macOS · SSH X11 图形环境" : `${esc(diagnostics.os_id || "Linux")} · ${esc(diagnostics.package_manager || "未识别包管理器")} · ${esc(diagnostics.display_manager || "未识别显示管理器")}`}</span></div><span class="status-pill">${macos ? state.sshX11?.ready ? "X11 已就绪" : state.sshX11?.xquartz_installed ? "XQuartz 已安装，配置未完成" : "未安装 XQuartz" : diagnostics.has_desktop ? `已发现 ${diagnostics.desktops.length} 个桌面` : "未检测到图形桌面"}</span></div>
      ${platformSupported ? `${renderLinuxDesktopSelectionSummary(diagnostics)}<div class="linux-desktop-grid">${catalog.map(item => { const id = item.id; const available = installable.has(id); const isInstalled = installed.has(id); const action = isInstalled ? "uninstall" : "install"; const enabled = (isInstalled || available) && !taskRunning; return `<article class="linux-desktop-card ${isInstalled ? "installed" : ""}" data-desktop-id="${escAttr(id)}"><div class="linux-desktop-card-icon">${icon(item.icon || "monitor")}</div><div><strong>${esc(item.label || linuxDesktopLabel(id))}</strong><span>${isInstalled ? "已安装，可用于图形会话" : available ? "可自动安装" : "当前包管理器未提供自动方案"}</span>${isInstalled ? renderLinuxDesktopUsageBadges(diagnostics, id) : ""}</div><button type="button" class="${enabled && !isInstalled ? "primary" : ""}" onclick="${isInstalled ? `uninstallLinuxDesktop('${escAttr(id)}')` : `installLinuxDesktop('${escAttr(id)}')`}" ${enabled ? "" : "disabled"}>${icon(isInstalled ? "trash-2" : "package-plus")}<span>${isInstalled ? "卸载" : "安装"}</span></button></article>`; }).join("")}</div>` : ""}
...
function openLinuxDesktopInstallOptions(desktopId) {
  ...
  const steps = [
    ...
    "本机下载后离线安装仅支持 Debian/Ubuntu 及兼容 APT/.deb 系统：TunnelDesk 会解析依赖、下载匹配架构的软件包并通过 SFTP 上传。",
    "使用远端缓存不会访问软件源，只有远端缓存已经完整时才能成功。",
    "安装结束后 TunnelDesk 会重新探测桌面会话；RDP、VNC 或 XDMCP 仍需各自的服务端配置。"
  ];
  modal.innerHTML = `<div class="modal-card wide remote-install-dialog" role="dialog" aria-modal="true" aria-labelledby="linuxDesktopInstallTitle">
    ...
    ${remoteInstallModesMarkup(plan, mode => `installLinuxDesktop('${escAttr(desktopId)}','${mode}',this)`)}
...
async function installLinuxDesktop(desktopId, mode="") {
  const id = Number(linuxDesktopManagerState.connectionId || 0);
  if (!id) return notify("请先选择 SSH 管理连接", "info");
  if (!mode) return openLinuxDesktopInstallOptions(desktopId);
  ...
  const message = normalizedMode === "local-offline"
    ? `TunnelDesk 将针对已识别的 Debian/Ubuntu 或兼容 APT/.deb 系统，在本机下载 ${linuxDesktopLabel(desktopId)} 及其依赖，通过 SFTP 上传后安装。是否继续？`
  ...
  if (!await confirmModal(message, `${modeLabel}安装 Linux 桌面`, "安装", "取消", true)) return;
  let adminAuth = null;
  ...
  try {
    ...
    renderLinuxDesktopManager();
    if (typeof refreshSftpJobs === "function") void refreshSftpJobs();
    pollLinuxDesktopInstallTask(task.id);
  } catch (error) {
    notify(error.message || `${modeLabel}安装 Linux 桌面启动失败`, "error");
  }
}

async function uninstallLinuxDesktop(desktopId) {
  const id = Number(linuxDesktopManagerState.connectionId || 0);
  if (!id) return notify("请先选择 SSH 管理连接", "info");
  const label = linuxDesktopLabel(desktopId);
  if (!await confirmModal(`将通过 SSH 卸载 ${label} 的桌面组件。可能影响现有图形登录、RDP、VNC 或 XDMCP 会话，是否继续？`, "卸载 Linux 桌面", "卸载", "取消", true)) return;
  let adminAuth = null;
  if (!linuxDesktopManagerState.diagnostics?.privileged) {
...
function pollLinuxDesktopInstallTask(taskId) {
  clearInterval(linuxDesktopManagerPollTimer);
  const poll = async () => {
    try {
      const task = await api(`/api/linux-desktop/tasks/${encodeURIComponent(taskId)}`);
      ...
      if (["done", "failed"].includes(task.status)) {
        ...
        if (task.status === "done") {
          ...
          linuxDesktopManagerState.taskId = "";
          renderLinuxDesktopManager();
          notify(task.error || `Linux 桌面${task.action === "uninstall" ? "卸载" : "安装"}失败`, "error");
        }
      }
    } catch (error) {
      clearInterval(linuxDesktopManagerPollTimer);
      linuxDesktopManagerPollTimer = null;
      notify(error.message || "读取桌面管理日志失败", "error");
    }
  };
  void poll();
  linuxDesktopManagerPollTimer = setInterval(poll, 900);
}
...
async function openRemoteDesktop(id, updateTab=true) {
  const profile = remoteProfileById(id);
  ...
  if (existingVncSession?.workspace && (existingVncSession.connected || existingVncSession.connecting)) return renderEmbeddedVnc(profile, key);
  const view = $("view-remote-desktop");
  if (!view) return;
  ...
  const helpText = embeddedXdmcp
    ? "TunnelDesk 负责启动本机 XDMCP 窗口；XDMCP 每次都会新建图形登录，不能接入已经打开的桌面，也不依赖 SSH X11 转发。需要共享当前桌面时请使用 VNC。"
    : managedRdp
      ? "TunnelDesk 会先探测远端 xrdp、桌面会话和本机客户端；默认停留在本页，由你确认后再打开远程桌面。"
      : `TunnelDesk 会先探测远端 VNC 服务和桌面环境；默认停留在本页，由你确认后再打开${embeddedVnc ? "内置" : "系统"}客户端。`;
  ...
  try {
    const [diagnostics, serverState, desktopDiagnostics] = await Promise.all([
      embeddedVnc ? Promise.resolve({vnc:{available:true, launchable:true, client:"TunnelDesk 内置 VNC"}}) : api("/api/remote-clients/diagnostics"),
      embeddedXdmcp
    ...
    const vncServiceBlocked = managedVnc && serverState?.diagnostics_available !== false && ["not-installed", "stopped", "not-listening", "blocked"].includes(vncStatus);
    ...
    const vncServerReady = !managedVnc || serverState?.diagnostics_available === false || !vncServiceBlocked;
    await withRemoteDesktopRenderScope(renderScope, async activeView => {
      ...
      if (updateTab && remoteDesktopQuickOpen && clientLaunchable && (!effectiveDesktopDiagnostics || effectiveDesktopDiagnostics.platform_supported === false || effectiveDesktopDiagnostics.has_desktop) && (!embeddedXdmcp || serverState?.ready_for_login) && rdpServerReady && vncServerReady) {
...
function renderRdpServerState(diagnostics, profileId=selectedRemoteProfileId, key=`remote-desktop-${profileId}`, targetContainer=null) {
  ...
  const detail = !linux
    ? "TunnelDesk 不会在非 Linux 主机上安装 xrdp；仍可直接使用系统 RDP 服务。"
    : installed
...
async function openRdpSetupGuide(profileId) {
  ...
  try {
    ...
    const steps = diagnostics.platform_supported === false
      ? ["当前 SSH 管理主机不是 Linux，TunnelDesk 不会安装 xrdp。", "Windows 主机请在系统设置中开启远程桌面；其他系统请使用其原生 RDP 服务端说明。"]
      : [
    ...
    modal.innerHTML = `<div class="modal-card wide x11-install-guide remote-install-dialog" role="dialog" aria-modal="true" aria-labelledby="rdpSetupGuideTitle">
      ...
      ${diagnostics.platform_supported === false || installed ? "" : remoteInstallModesMarkup(plan, mode => `installRdpServer(${Number(profileId)},'remote-desktop-${Number(profileId)}',this,'${mode}')`)}
      ${remoteInstallManualMarkup(plan, {steps, note:"如果远端和本机都无法联网，请在另一台同发行版、同版本、同架构的设备上下载 xrdp、xorgxrdp 及完整依赖，再上传到远端安装。"})}
      <div class="actions"><button type="button" onclick="closeRdpSetupGuide()">关闭</button>${installed ? `<button type="button" onclick="runRdpServerAction(${Number(profileId)},'remote-desktop-${Number(profileId)}','${active ? "stop" : "start"}',this)">${icon(active ? "circle-stop" : "circle-play")}<span>${active ? "停止服务" : "启动服务"}</span></button><button class="danger" type="button" onclick="runRdpServerAction(${Number(profileId)},'remote-desktop-${Number(profileId)}','uninstall',this)">${icon("package-minus")}<span>卸载服务</span></button>` : ""}${sourceId ? `<button type="button" onclick="closeRdpSetupGuide();openLinuxDesktopManager(${sourceId})">${icon("monitor-cog")}<span>Linux 桌面管理</span></button>` : ""}</div>
    </div>`;
    setRemoteInstallDialogCommands(plan);
    modal.hidden = false;
    modal.onclick = null;
...
async function installRdpServer(profileId, key, button=null, mode="online") {
  ...
  const message = normalizedMode === "local-offline"
    ? "TunnelDesk 将针对已识别的 Debian/Ubuntu 或兼容 APT/.deb 系统，在本机下载匹配架构的 xrdp 软件包及依赖，通过 SFTP 上传后安装。是否继续？"
    : normalizedMode === "offline"
  ...
  try {
    ...
    if (result.task) {
      ...
      closeRdpSetupGuide();
      watchRemoteComponentTask(result.task, {
      ...
      notify("RDP 服务安装已加入任务中心", "success");
      return result;
    }
...
function renderXdmcpServerState(diagnostics, profileId=selectedRemoteProfileId, targetContainer=null) {
  ...
  const firewallLabel = diagnostics.firewall === "none"
    ...
    : diagnostics.firewall_managed
      ? `${diagnostics.firewall} · TD 已管理 177/UDP`
  ...
  const actionHandler = action
    ...
    : "";
  const serviceActions = `<div class="remote-service-actions">${action ? `<button class="${action.danger ? "danger" : "primary"}" onclick="${actionHandler}">${icon(action.icon)}<span>${esc(action.label)}</span></button>` : ""}${diagnostics.can_uninstall_lightdm || diagnostics.tunneldesk_backup_available ? `<button class="danger" type="button" onclick="configureXdmcpHost(${Number(profileId || 0)},'uninstall-lightdm',this)">${icon("package-minus")}<span>卸载并恢复显示管理器</span></button>` : ""}</div>`;
...
async function openXdmcpSetupGuide(profileId, requestedAction="") {
  ...
  try {
    ...
    const steps = isMac
      ? ["macOS 远端不提供可由 TunnelDesk 自动启用的 XDMCP 显示管理器。请在 Linux 服务器上使用 XDMCP，macOS 服务器建议使用 VNC。", "如果只是运行 X11 应用，请在本机安装并启动 XQuartz，然后使用 SSH X11 转发。"]
      : needsDesktopInstall
        ...
        : needsManagerInstall
          ? ["当前桌面会话可用，但现有显示管理器不提供可用的 XDMCP 服务端。", "Debian/Ubuntu 可由 TunnelDesk 安装并切换到 LightDM；该操作会结束当前图形登录，请先保存工作。", "切换完成后重新探测，再启动图形登录。", "XDMCP 使用 UDP 177，只应在可信局域网开放。"]
          : diagnostics.listening
            ...
            : diagnostics.supported && diagnostics.has_x11_session
              ? [`已检测到 ${manager} 和 ${desktopLabel}，桌面组件已经可用。`, "当前缺少的是 XDMCP 服务开关：返回连接页面点击“启用 XDMCP 服务”，无需重新安装 LightDM 或桌面环境。", "启用时 TunnelDesk 只会在检测到活动防火墙时开放 UDP 177；未启用防火墙不会添加多余规则。", "XDMCP 不加密，只应在可信局域网使用。"]
    ...
    modal.innerHTML = `<div class="modal-card wide x11-install-guide remote-install-dialog" role="dialog" aria-modal="true" aria-labelledby="xdmcpSetupGuideTitle">
      ...
      ${installPlan ? remoteInstallModesMarkup(installPlan, mode => `configureXdmcpHost(${Number(profileId)},'${installAction}',this,'${mode}')`) : ""}
...
async function configureXdmcpHost(id, action, button=null, installMode="online") {
  ...
  const message = cleanupSessions
    ...
    : uninstallLightdm
    ? "将停止 XDMCP、卸载由 TunnelDesk 安装的 LightDM，并尝试恢复切换前的显示管理器。当前图形会话会结束；只有检测到可恢复备份时才会自动执行。"
    : install
  ...
  try {
    ...
    if (result.task) {
      ...
      notify(`${result.task.action_label || result.task.component_label || "XDMCP 操作"}已加入任务中心`, "success");
      return result;
    }
...
function requestVncClipboardSshConnection(session) {
  return new Promise(resolve => {
    ...
    modal.innerHTML = `<form class="modal-card vnc-clipboard-modal vnc-clipboard-helper-modal">
      ...
      <div class="connection-test-status">VNC 的旧剪贴板协议只能可靠传输 Latin-1。关联 SSH 后，TunnelDesk 会直接读写远端系统剪贴板，中文使用 UTF-8 传输。</div>
...
function renderVncClipboardHelperState(session, diagnostics, container) {
  ...
  if (platform === "macos") {
    ...
    body = `${remoteInstallModesMarkup(diagnostics.install_plan || {}, mode => `installVncClipboardHelper('${escAttr(session.key)}','${escAttr(mode)}',this)`)}<div class="connection-test-status warning">${icon("info")}<span>${esc(diagnostics.reason || "请安装与当前 X11/Wayland 会话匹配的剪贴板工具。")}</span></div>`;
  } else if (available) {
    ...
    const uninstallAction = platform === "linux" && uninstallPlan.available
      ? `<div class="actions tight"><button class="danger" type="button" onclick="uninstallVncClipboardHelper('${escAttr(session.key)}',this)">${icon("package-minus")}<span>卸载辅助工具</span></button></div>`
      : "";
...
async function installVncClipboardHelper(key, mode="online", button=null) {
  ...
  try {
    ...
    if (result.task) {
      ...
      resetVncClipboardTransportState(session);
      setVncClipboardStatus(session, "SSH 剪贴板辅助安装中", "active");
      notify(`剪贴板辅助工具${modeLabel}安装已加入任务中心`, "success");
      if (typeof refreshSftpJobs === "function") void refreshSftpJobs();
      void watchVncClipboardHelperTask(session, result.task.id, action);
      return result;
    }
    resetVncClipboardTransportState(session);
...
async function uninstallVncClipboardHelper(key, button=null) {
  ...
  try {
    ...
    if (result.task) {
      stopVncClipboardPolling(session);
      resetVncClipboardTransportState(session);
      setVncClipboardStatus(session, "SSH 剪贴板辅助卸载中", "active");
      notify("剪贴板辅助工具卸载已加入任务中心", "success");
      if (typeof refreshSftpJobs === "function") void refreshSftpJobs();
      void watchVncClipboardHelperTask(session, result.task.id, "uninstall");
      return result;
    }
    resetVncClipboardTransportState(session);
...
async function readVncLocalClipboard() {
  if (window.tunnelDeskDesktop?.readClipboardText) return String(await window.tunnelDeskDesktop.readClipboardText());
...
async function writeVncLocalClipboard(text, manual=false) {
  const value = String(text ?? "");
  if (window.tunnelDeskDesktop?.writeClipboardText) {
    await window.tunnelDeskDesktop.writeClipboardText(value);
    return;
  }
  if (!manual) {
...
function requestVncClipboardText(session, reason="") {
  return new Promise(resolve => {
    ...
    modal.innerHTML = `<form class="modal-card vnc-clipboard-modal"><h2>发送剪贴板到 ${esc(session?.profile?.name || "VNC")}</h2>
      ...
      <div class="muted">内容只发送到当前 VNC 会话，不会保存到 TunnelDesk。</div>
...
function renderVncServerState(diagnostics, profileId=selectedRemoteProfileId, key=`remote-desktop-${profileId}`, targetContainer=null) {
  ...
  setRemoteComponentTaskHost(container, false);
  container._vncDiagnostics = diagnostics || {};
  if (diagnostics?.error) {
    container.innerHTML = `<div class="connection-test-status warning">${icon("circle-alert")}<span>${esc(diagnostics.error)}</span></div>`;
  } else {
    container.innerHTML = vncConnectionHelpMarkup(profile, diagnostics?.platform || "", vncServerReady(diagnostics), "", diagnostics, key, {preflight:true, showConnect:false});
  }
  const status = String(diagnostics?.status || "").toLowerCase();
  const blocked = diagnostics?.diagnostics_available !== false && ["not-installed", "stopped", "not-listening", "blocked"].includes(status);
  const launchButton = remoteWorkspaceQuery(container, "#remoteDesktopLaunchButton", "remoteDesktopLaunchButton");
  const view = remoteWorkspaceQuery(container, "#view-remote-desktop", "view-remote-desktop");
...
function vncDiagnosticCopy(diagnostics, serviceAvailable, detail="") {
  ...
  if (status === "not-installed") return {title:"未安装 VNC 服务", summary:"远端 Linux 主机没有检测到 VNC Server 组件，可以授权 TunnelDesk 安装，或按手册手动安装。"};
...
function vncConnectionHelpMarkup(profile, platform="", serviceAvailable=false, detail="", diagnostics=null, key="", options={}) {
  ...
  const installed = Boolean(diagnostics?.installed || diagnostics?.service_unit || diagnostics?.commands?.length);
  const running = Boolean(serviceAvailable || diagnostics?.listening || ["ready", "reachable"].includes(status));
  const copy = options.preflight && running
    ? {title:"VNC 服务已就绪", summary:"远端 VNC 端口和服务探测通过，可以打开远程桌面。"}
    : vncDiagnosticCopy(diagnostics, serviceAvailable, detail);
  const needsInstall = status === "not-installed" || (["stopped", "not-listening"].includes(status) && !diagnostics?.start_plan);
  const installModes = needsInstall && !macos
    ? remoteInstallModesMarkup(installPlan, mode => `installVncServer(${Number(profile.id)},'${escAttr(reconnectKey)}',this,'${mode}')`, `openVncSetupGuide(${Number(profile.id)})`)
    : "";
  const startButton = ["stopped", "not-listening", "blocked"].includes(status) && diagnostics?.start_plan
    ? `<button class="primary" type="button" onclick="startVncServer(${Number(profile.id)},'${escAttr(reconnectKey)}',this)">${icon("play")}<span>${status === "blocked" ? "放行并启动" : diagnostics.start_plan.kind === "service" ? "启动 VNC 服务" : diagnostics.start_plan.kind === "tigervnc-systemd" ? "配置并启用 TigerVNC" : "配置并启动"}</span></button>`
    : "";
  const serviceActions = installed && !macos ? `${startButton ? "" : `<button type="button" onclick="runVncServerAction(${Number(profile.id)},'${escAttr(reconnectKey)}','${running ? "stop" : "start"}',this)">${icon(running ? "circle-stop" : "circle-play")}<span>${running ? "停止服务" : "启动服务"}</span></button>`}<button class="danger" type="button" onclick="runVncServerAction(${Number(profile.id)},'${escAttr(reconnectKey)}','uninstall',this)">${icon("package-minus")}<span>卸载服务</span></button>` : "";
  ...
  return `<div class="remote-service-state vnc-connection-help-panel">
    ...
    ${diagnostics?.service_unit ? `<div class="vnc-setup-legacy">服务单元：${esc(diagnostics.service_unit)} · 状态：${esc(diagnostics.service_state || "未知")}${diagnostics?.start_plan?.persistent ? " · TunnelDesk 管理 · 重启后自动启动" : ""}</div>` : ""}
    ${installModes}
    <div class="remote-service-actions">
      ${startButton}${serviceActions}
...
async function openVncSetupGuide(profileId) {
  ...
  try {
    ...
    const installed = Boolean(diagnostics.installed || diagnostics.service_unit || diagnostics.commands?.length);
    const running = Boolean(diagnostics.listening || ["ready", "reachable"].includes(String(state).toLowerCase()));
    const key = vncSessionKeyForProfile(profileId);
    const installPlan = diagnostics.install_plan || {};
    const installModes = !installed && diagnostics.platform !== "macos"
      ? remoteInstallModesMarkup(installPlan, mode => `installVncServer(${Number(profileId)},'${escAttr(key)}',this,'${mode}')`)
      : "";
    const serviceActions = installed && diagnostics.platform !== "macos" ? `<button type="button" onclick="runVncServerAction(${Number(profileId)},'${escAttr(key)}','${running ? "stop" : "start"}',this)">${icon(running ? "circle-stop" : "circle-play")}<span>${running ? "停止服务" : "启动服务"}</span></button><button class="danger" type="button" onclick="runVncServerAction(${Number(profileId)},'${escAttr(key)}','uninstall',this)">${icon("package-minus")}<span>卸载服务</span></button>` : "";
    modal.innerHTML = `<div class="modal-card wide x11-install-guide vnc-setup-guide" role="dialog" aria-modal="true" aria-labelledby="vncSetupGuideTitle">
      ...
      ${installModes}
      ${remoteInstallManualMarkup(installPlan, {steps, commands, note:"TunnelDesk 只会在你明确授权后执行安装、启停或卸载。VNC 密码不会写入命令、日志或本窗口；本机也无法联网时，请在其他匹配远端系统版本与 CPU 架构的设备准备软件包和全部依赖。"})}
...
async function runVncServerAction(profileId, key, action, button=null) {
  ...
  const message = isInstall
    ? localOfflineInstall
      ? "TunnelDesk 将根据远端 Debian/Ubuntu 或兼容 APT/.deb 系统的软件包索引，在本机下载匹配架构的软件包和依赖，再通过 SFTP 上传并使用临时管理员权限安装。是否继续？"
      : `TunnelDesk 将在远端执行${offlineInstall ? "缓存离线" : "在线"}安装。安装不会自动覆盖现有 VNC 配置，是否继续？`
    : uninstall
      ? "将停止并卸载远端 VNC Server 与剪贴板辅助软件包。不会卸载 Linux 桌面环境，但现有 VNC 会话会立即断开。是否继续？"
      : stop
        ? "将停止远端 VNC 服务，现有 VNC 会话会立即断开。是否继续？"
        : "TunnelDesk 将尝试启动检测到的 VNC 服务单元，是否继续？";
  ...
  try {
    ...
    if (saveVncPasswordAfterConfirm) {
      try {
    ...
    if (result.task) {
      ...
      if (session) vncSessionStatus(session, `${actionLabel}已加入任务中心`, "connecting");
      watchRemoteComponentTask(result.task, {
      ...
      notify(`${actionLabel}已加入任务中心`, "success");
      return result;
    }
...
async function openEmbeddedVncDesktop(profileId, key=`remote-desktop-${profileId}`, button=null) {
  ...
  try {
    ...
    if (desktopDiagnostics?.platform_supported !== false && desktopDiagnostics && !desktopDiagnostics.has_desktop) {
      renderLinuxDesktopMissingWorkspace(profile, key, desktopDiagnostics);
      return null;
    }
    renderEmbeddedVnc(profile, key, diagnostics || desktopDiagnostics);
    return true;
  } catch (error) {
    notify(error.message || "内置 VNC 打开失败", "error");
    return null;
  } finally {
    if (button && document.contains(button)) setButtonBusy(button, false);
  }
...
function renderEmbeddedVnc(profile, key, diagnostics=null) {
  ...
  if (!session) {
    session = {key, profile, rfb:null, screen:document.createElement("div"), status:null, statusText:"", statusState:"connecting", connecting:false, connected:false, clipboardAutoSync:!profile.options?.view_only, remoteClipboardAvailable:false, remoteClipboardPending:false};
...
async function toggleVncClipboardSync(key) {
  ...
  try {
    ...
    notify("已开启 VNC 剪贴板自动同步；仅在 TunnelDesk 位于前台时读取本机剪贴板", "success");
  } catch (error) {
    session.clipboardAutoSync = false;
    session.clipboardPermissionBlocked = true;
    stopVncClipboardPolling(session);
      ...
      if (serverState.session_conflict) {
        if (!serverState.can_cleanup_remote_sessions) {
          throw new Error(serverState.local_graphical_sessions?.length
            ...
            : "Plasma 已绑定到其他图形会话，TunnelDesk 无法安全自动结束；请先注销旧会话。");
        }
...
async function installRemoteDesktopClient(profileId, protocol, button=null) {
  ...
  try {
    ...
    if (result.opened) {
      notify(`已打开 ${result.target || "客户端下载页面"}，安装完成后返回 TunnelDesk 重新检测`, "info");
      return result;
    }
...
async function deleteFtpEntry(key, name, type) {
  const state = ftpProfileStates.get(key);
  if (!state || !await confirmModal(`确定删除 ${name}${type === "directory" ? " 及其内容" : ""}？FTP 不支持 TunnelDesk 回收站。`, "删除 FTP 项目", "删除", "取消", true)) return;
...
async function openX11InstallGuide(connectionId) {
  ...
  try {
    ...
    modal.innerHTML = `<div class="modal-card wide x11-install-guide remote-install-dialog" role="dialog" aria-modal="true" aria-labelledby="x11InstallGuideTitle">
      ...
      ${remoteInstallModesMarkup(plan, mode => `installRemoteX11Components(${Number(connectionId)},this,${plan.requires_password ? "true" : "false"},'${mode}')`)}
...
async function installRemoteX11Components(connectionId, button=null, needsAuthorization=true, mode="online") {
  ...
  const modeLabel = normalizedMode === "local-offline" ? "本机下载后离线" : normalizedMode === "offline" ? "使用远端缓存" : "在线";
  const message = normalizedMode === "local-offline"
    ? "TunnelDesk 将针对已识别的 Debian/Ubuntu 或兼容 APT/.deb 系统，在本机下载匹配的软件包及依赖，通过 SFTP 上传后安装 X11 组件。是否继续？"
    : normalizedMode === "offline"
      ? "将只使用远端包管理器已经缓存的软件包安装 X11 组件，不会访问软件源。缓存不完整时安装会失败，是否继续？"
      : "将通过远端软件源在线安装 X11 组件。是否继续？";
  if (!await confirmModal(message, `${modeLabel}安装 X11 组件`, "安装", "取消", true)) return;
  const auth = needsAuthorization ? await requestRemoteAdminAuthorization(connectionId, `${modeLabel}安装远端 X11 组件`) : null;
  if (needsAuthorization && !auth) return;
  try {
    if (button && document.contains(button)) setButtonBusy(button, true, "安装中...");
    const action = normalizedMode === "local-offline" ? "install-local-offline" : normalizedMode === "offline" ? "install-offline" : "install";
    ...
    if (result.task) {
      ...
      notify("X11 组件安装已加入任务中心", "success");
    } else {
...
async function uninstallRemoteX11Components(connectionId, button=null) {
  ...
  try {
    ...
    if (result.task) {
      ...
      notify("X11 组件卸载已加入任务中心", "success");
      return result;
    }
    closeX11InstallGuide();
...
function xServerModeLabel(value) {
  return {bundled:"TunnelDesk 内置", system:"系统组件", native:"桌面会话", missing:"未安装"}[value] || value || "未知";
}
...
async function renderXServerManager() {
  ...
  const linuxX11Action = diagnostics.platform === "linux" && connection
    ...
    : "";
  const linuxHint = diagnostics.platform === "linux"
  ...
  modal.innerHTML = `<div class="modal-card xserver-manager" role="dialog" aria-modal="true" aria-labelledby="xServerManagerTitle">
    <div class="modal-title-row"><div><h2 id="xServerManagerTitle">X Server</h2><span class="muted">${esc(diagnostics.server || xServerModeLabel(diagnostics.mode))}</span></div><button class="icon-button" type="button" onclick="closeXServerManager()" title="关闭" aria-label="关闭">${icon("x")}</button></div>
    <div class="xserver-state ${diagnostics.available ? "ready" : diagnostics.running ? "warning" : ""}"><span>${icon(diagnostics.available ? "circle-check" : diagnostics.running ? "circle-pause" : "circle-alert")}</span><div><b>${esc(diagnostics.available ? "已就绪" : diagnostics.running ? "正在运行" : "未启动")}</b><small>${esc(diagnostics.reason || "")}</small></div></div>
    <dl class="xserver-details"><div><dt>运行方式</dt><dd>${esc(xServerModeLabel(diagnostics.mode))}</dd></div><div><dt>DISPLAY</dt><dd>${esc(diagnostics.display || "-")}</dd></div><div><dt>管理范围</dt><dd>${esc(diagnostics.managed ? "TunnelDesk 当前会话" : "系统或桌面会话")}</dd></div>${["win32","darwin"].includes(diagnostics.platform) ? `<div><dt>下次启动</dt><dd>${diagnostics.auto_start === false ? "保持关闭" : "自动启动"}</dd></div>` : ""}</dl>
    ${linuxHint}
    ${sshX11Block}
    <div class="actions"><button type="button" onclick="renderXServerManager()">${icon("refresh-cw")}<span>刷新</span></button>${diagnostics.can_stop ? `<button type="button" onclick="changeXServerState('stop',this)">${icon("square")}<span>停止</span></button>` : ""}${diagnostics.can_install ? `<button class="primary" type="button" onclick="installXServerComponentsFromManager(this,'${escAttr(diagnostics.platform || "")}')">${icon("download")}<span>${esc(installLabel)}</span></button>` : ""}${diagnostics.can_start ? `<button class="primary" type="button" onclick="changeXServerState('start',this)">${icon("play")}<span>启动</span></button>` : ""}${linuxX11Action}</div>
  </div>`;
  modal.hidden = false;
  refreshIcons();
}
...
async function changeSshX11Forwarding(action, button, connectionId, source="xserver") {
  ...
  if (!await confirmModal(message, enabled ? "开启 SSH X11 转发" : "关闭 SSH X11 转发", enabled ? "开启" : "关闭", "取消", enabled ? false : true)) return;
  let adminAuth = null;
  try {
    ...
    await api(`/api/connections/${Number(connectionId)}/x11-forwarding`, {method:"POST", body:JSON.stringify({action, ...(adminAuth ? {admin_auth:adminAuth} : {})})});
...
async function changeXServerState(action, button) {
  if (action === "stop" && !await confirmModal("停止 X Server 会同时关闭由它显示的 X11/XDMCP 图形窗口，并在下次启动 TunnelDesk 时保持关闭。", "停止 X Server", "停止", "取消", true)) return;
  try {
