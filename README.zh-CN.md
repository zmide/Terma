...
- 支持在 SSH 终端中直接使用 ZMODEM `sz` 下载和 `rz` 上传文件；`rz` 上传前检查远端同名文件并提供覆盖、自动重命名或取消，显示实际文件名与传输进度，并可通过按钮或 `Ctrl+C` 取消。
...
- 字体、字号、行距和字重可按连接保存，支持按住 `Ctrl` 使用鼠标滚轮调整字号。
- 最近命令、快捷键栏、终端快速命令栏、右键操作、终端日志和交互延迟显示；快速命令栏可收为紧凑单行、独立滚动，默认使用无图标短按钮，也可选中文徽标并单击执行或仅插入终端。
...
- 文件和目录浏览、搜索、排序、分页、收藏、上传、下载和在线文本编辑；搜索支持 `Ctrl+F`、加载反馈和按需包含子目录。
...
- 桌面端可使用系统程序、VS Code 或自定义编辑器修改远端文件；内容变化后会询问是否保存，文本冲突可先在左右对比视图中检查，随后选择备份覆盖、另存为或暂不保存。内部编辑器可比较最近 10 个备份版本。
...
- RDP 使用系统原生客户端，由 SSH 生成的配置会按目标地址直接打开并由客户端询问桌面账号；macOS 未安装 Windows App 时可使用 Microsoft 官方独立 PKG，支持提前放入下载目录后离线安装。VNC 自动优先使用随包提供的 TigerVNC Viewer，再回退到内置 noVNC 工作区，最后回退系统客户端；三种方式都可以单独选择。VNC 支持可选加密保存密码，并在工作区内完成控制、双向剪贴板自动同步、全屏和重连；鼠标默认按远端平台自动选择显示策略，也可在 VNC 工具栏手动显示或隐藏本地光标。权限受限时仍可手动收发剪贴板，密码错误时可重新输入并更新保存密码。
...
- 每条转发规则独立启停，可使用模板、可选 URL 路径、自动重连和启动恢复。
- “转发列表”可跨全部 SSH 连接集中管理规则，支持搜索、状态/服务器筛选、不分组或按 SSH 查看、分组/服务器标题冻结、拖动排序，以及直接新增、编辑、删除和访问。
- 两套“快速打开”都可按连接、服务、备注、地址、端口、状态或 URL 路径查找转发，并直接启动或停止。
...
- SSH 密码、私钥路径、访问 Token 和数据库备份属于敏感数据，请保护运行数据目录并按需启用配置加密。配置加密使用独立的加密密钥与密码校验值；旧版 v1/v2 配置会在输入原主密码解锁后使用新盐轮换到 `termaenc:v3:`，不需要重新录入连接凭据。轮换完成后请重新生成备份并清理旧版 v1 备份；若旧备份曾泄露，还应轮换其中保存的 SSH、VNC 等实际凭据。
- 启用配置加密后，重启进入锁定状态时仍可查看普通列表和调整非敏感显示设置，但不能下载包含完整凭据的加密迁移包，也不能恢复数据库或快照、完整编辑连接、保存凭据和终端启动配置；输入主密码解锁后恢复正常。启用或关闭过程中若意外断电，Terma 会保留待继续状态并阻止使用敏感字段，重新输入主密码即可完成修复。
- 配置 Web 密码或访问 Token 后，普通浏览器即使从本机访问也需要登录，登录页可输入其中任一种凭据；Electron 桌面端使用每次启动随机生成的独立凭据。
- SSH 私钥只接受当前运行密钥目录或用户 `~/.ssh` 顶层的真实、独立普通私钥文件，拒绝目录、外部路径、软硬链接、公钥和配置文件；SSH 附加参数只允许算法、压缩、超时等连接调优选项，不能改写主机、用户、跳板、转发、X11、本机程序、凭据或主机信任边界。升级前已保存的不合规私钥路径或附加参数会在编辑连接时明确提示调整，不会自动移动或修改原文件。
...
- Windows、Linux 和 macOS 会主动收紧数据目录、数据库、安全配置、快照和恢复暂存文件的本机权限；关键目录无法安全落盘时会拒绝启动并指出具体路径，避免在共享目录或异常 ACL 下静默运行。
