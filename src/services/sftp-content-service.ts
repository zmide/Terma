...
function prepareSftpWriteContent(content, encoding = "utf8", remotePath = "", requestedLineEnding = "") {
  ...
  const lineEnding = unixScript ? "lf" : (["lf", "crlf", "cr"].includes(requestedLineEnding) ? requestedLineEnding : "");
  ...
  if (unixScript && text && !text.endsWith("\n")) text += "\n";
  ...
  return {content:encoded, maximum_bytes:maximumBytes, encoding:selectedEncoding, line_ending:lineEnding || null, normalized_script:unixScript};
}
