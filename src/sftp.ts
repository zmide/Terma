...
function buildRemotePagedDirectoryEntriesCommand(options: any = {}) {
  ...
  return [
    ...
    metadataEmitter,
    `terma_entries() { if [ -n "$TERMA_QUERY" ]; then ${listedEntries} | LC_ALL=C grep -a -F -i -- "$TERMA_QUERY"; else ${listedEntries}; fi; }`,
    fastMetadataEmitter,
