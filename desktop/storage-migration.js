...
function runtimeRoot(paths) {
  ...
  if (path.basename(dataDir) !== "data" || path.basename(sshDir) !== ".ssh") {
    throw new Error("数据迁移路径结构无效");
  }
  const dataRoot = path.dirname(dataDir);
  const sshRoot = path.dirname(sshDir);
  if (dataRoot !== sshRoot) throw new Error("数据与密钥目录必须位于同一运行根目录");
  return dataRoot;
}
...
function transitionPaths(transition) {
  ...
  if (!/^[0-9a-f]{32}$/.test(id)) throw new Error("数据迁移标识无效");
  ...
  if (!path.isAbsolute(sourceRoot) || !path.isAbsolute(targetRoot) || sourceRoot === targetRoot) {
    throw new Error("数据迁移源路径或目标路径无效");
  }
  if (pathInside(sourceRoot, targetRoot)) throw new Error("新数据目录不能位于当前数据目录内部");
  return {
    id,
    sourceRoot,
    targetRoot,
...
function assertDesktopStorageMigrationTarget(transition) {
  ...
  for (const name of ["data", ".ssh"]) {
    if (fs.existsSync(path.join(paths.targetRoot, name))) {
      throw new Error(`目标目录已经包含 ${name} 数据；立即迁移只允许使用没有现有 Terma 数据的目录`);
    }
  }
  ...
  if (otherMarkers.length) throw new Error("目标目录存在未完成的数据迁移，请先恢复原路径或清理对应迁移提示");
  return paths;
}
...
function completeDesktopStorageTransition(transition, options = {}) {
  if (Number(transition?.version) !== TRANSITION_VERSION) throw new Error("数据迁移状态版本不受支持");
  if (typeof options.copyRuntimeDirectory !== "function") throw new Error("数据迁移复制器不可用");
  const paths = transitionPaths(transition);
  fs.mkdirSync(paths.targetRoot, {recursive:true});
  ...
  if (marker?.id === paths.id && marker?.state === "promoted") {
    ...
    if (JSON.stringify(sourceManifest) !== JSON.stringify(targetManifest)) throw new Error("已迁移数据与原目录的文件清单不一致");
    return paths;
  }
  if (marker && marker.id !== paths.id) throw new Error("目标目录属于另一项未完成的数据迁移");
  ...
  if (JSON.stringify(sourceManifest) !== JSON.stringify(stagedManifest)) {
    throw new Error("迁移后的文件清单与原数据不一致");
  }
  ...
  for (const name of ["data", ".ssh"]) {
    ...
    writeMarker(paths.markerFile, promotion);
    if (fs.existsSync(destination)) throw new Error(`目标目录在迁移期间出现了 ${name} 数据，已停止以避免覆盖`);
