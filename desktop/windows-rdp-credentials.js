...
function windowsRdpCredentialTarget(endpoint) {
  ...
  if (!value || /[\0\r\n]/.test(value)) throw new Error("RDP 凭据目标无效");
...
function launchWindowsRdpWithCredential(options = {}) {
  ...
  if (typeof runtimeSpawn !== "function") throw new Error("Windows RDP 启动器不可用");
  ...
  if (!username) throw new Error("传递 RDP 密码时必须填写用户名");
  if (!password) throw new Error("没有可传递的 RDP 密码");
  if ([executable, rdpFile, username, password].some(value => /\0/.test(value))) throw new Error("RDP 凭据包含无效控制字符");
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("spawn", () => {
      ...
      timer = setTimeout(() => finish(new Error("Windows RDP 凭据交接超时，请重试")), 10000);
      timer.unref?.();
    });
    child.once("error", error => finish(error));
    child.once("close", code => {
      if (!settled) finish(new Error(stderr.trim() || stdout.trim() || `Windows RDP 凭据交接失败（代码 ${code ?? "未知"}）`));
    });
  });
}
