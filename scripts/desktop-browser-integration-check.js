...
async function main() {
  ...
  assert.equal(browserSessionGrant.expiresAt, 0);
  ...
  const browserSessionStatus = security.desktopBrowserGrantStatus(mockRequest({cookie:`${sessionCookie}; ${browserSessionCookie}`}));
  assert.equal(browserSessionStatus.authorized, true);
  assert.equal(browserSessionStatus.browser_session, true);
  ...
  const backend = serverModule.startServer(
    serverModule.parseArgs(["--host", "127.0.0.1", "--port", String(port)]),
    {
      ...
      desktopIntegration:{
        confirmDesktopBrowserAuthorization:async requestDetails => {
  ...
  try {
    ...
    assert.deepEqual(promptRequests.map(item => item.scopes), [["xserver"], ["xserver", "remote-client"], ["xserver"]]);
  } finally {
