...
const root = fs.mkdtempSync(path.join(os.tmpdir(), "tunneldesk-key-check-"));
...
try {
  ...
  const first = generateSshKey({name:"id_ed25519_check", comment:"TunnelDesk regression", passphrase:"test-passphrase"});
  const second = generateSshKey({name:"id_ed25519_check", comment:"TunnelDesk regression"});
  ...
  for (let index = 0; index < 32; index += 1) {
    const generated = generateSshKey({name:"id_ed25519_stress", comment:`TunnelDesk stress ${index}`, passphrase:"test-passphrase"});
