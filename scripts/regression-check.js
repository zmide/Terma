...
async function main() {
  ...
  ok("快速打开可搜索连接与转发并限制大结果集重绘", connectionsFrontend.includes("function quickForwardRows") && connectionsFrontend.includes("quickForwardSearchText") && connectionsFrontend.includes("QUICK_FORWARD_RESULT_LIMIT = 120") && connectionsFrontend.includes("requestAnimationFrame") && connectionsFrontend.includes('data-dblclick-action="quick-connect-forward"') && connectionsFrontend.includes('data-forward-id="${forward.id}"') && connectionsFrontend.includes("url_path"));
