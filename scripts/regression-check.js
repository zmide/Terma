...
const checks = [];
const frontendFiles = [
  "public/app-api.js", "public/app-utils.js", "public/app-workspace.js", "public/app-docking.js", "public/app-settings.js",
  "public/app-running.js", "public/app-batch.js", "public/app-logs.js", "public/app-connections.js",
  "public/app-terminal.js", "public/app-forwards.js", "public/app-import.js", "public/app-sftp.js",
  "public/app-sftp-tasks.js", "public/app-local-files.js", "public/app-actions.js", "public/app-productivity.js",
  "public/app-remote.js", "public/app.js"
];
...
async function main() {
  ...
  const sftpFrontend = ["public/app-sftp.js", "public/app-sftp-tasks.js"].map(read).join("\n");
  ...
  ok("任务中心接入 Linux 桌面安装与卸载任务", sftpFrontend.includes('/api/linux-desktop/tasks') && sftpFrontend.includes('type:"linux-desktop"') && sftpFrontend.includes("openLinuxDesktopTask") && read("public/app-remote.js").includes("if (typeof refreshSftpJobs === \"function\") void refreshSftpJobs()") && read("src/server.ts").includes("clearFinishedLinuxDesktopTasks"));
  ...
  const remoteFrontend = read("public/app-remote.js");
  const forwardsFrontend = read("public/app-forwards.js");
  const runningFrontend = read("public/app-running.js");
  const logsFrontend = read("public/app-logs.js");
  const connectionsFrontend = read("public/app-connections.js");
  ...
  const serverSource = ["src/server.ts", "src/storage-restore.ts"].map(read).join("\n");
  ...
  ok("终端断开后支持按 Enter 重连并保持标签级日志", terminalFrontend.includes("连接已关闭，按 Enter 重新连接") && terminalFrontend.includes("terminalReconnectInput(data)") && terminalFrontend.includes("log_id=") && terminalFrontend.includes("logId:createTerminalLogId()") && terminalSource.includes('url.searchParams.get("log_id")') && read("src/logs.ts").includes("terminalLogIdentity") && read("src/logs.ts").includes("重新连接时间"));
  ...
  ok("终端日志列表单独完整显示日期和时间", read("public/app-logs.js").includes("terminalLogPresentation") && read("public/app-logs.js").includes('class="log-item-time"') && appCss.includes(".log-item-time") && appCss.includes("white-space:nowrap"));
  ...
  ok("其他协议编辑页的关闭按钮会关闭当前工作区标签", remoteFrontend.includes('onclick="closeTabsByKey([activeTabKey],activeTabKey)">关闭</button>') && !remoteFrontend.includes('renderConnections();backToExplorer()">取消</button>'));
  ...
  ok("连接健康状态使用主题图标且保活字段保持紧凑", read("public/app-connections.js").includes('class="health-badge${healthClass}" title="健康状态：') && !read("public/app-connections.js").includes('${icon(health?.ok ? "circle-check" : health ? "circle-alert" : "circle-help")} ${esc(healthText)}') && indexHtml.includes("保活间隔（秒）") && indexHtml.includes('id="conn_keepalive_interval_help"'));
  ...
  ok("终端工具栏桌面端使用纯图标并在窄屏分行保留全部按钮", appCss.includes("container-name:terminal-view") && appCss.includes("@container terminal-view (max-width:1080px)") && appCss.includes("@container terminal-toolbar (max-width:1080px)") && appCss.includes("@media (min-width:761px) and (hover:hover) and (pointer:fine)") && appCss.includes(".terminal-actions > button > span:not(.composite-icon)") && terminalFrontend.includes('title="转发列表"') && terminalFrontend.includes('icon("earth")'));
  ok("X11、连接快捷入口和窗口标题保持统一", utilsFrontend.includes('name === "x11"') && settingsFrontend.includes('icon("x11")') && terminalFrontend.includes('icon("x11")') && read("public/app-productivity.js").includes('class="xserver-x-icon"') && read("public/app-productivity.js").includes('<ellipse') && !read("public/app-productivity.js").includes('button.innerHTML = icon("x11")') && connectionsFrontend.includes('onclick="openSftp(${c.id})"') && connectionsFrontend.includes('ondblclick="event.stopPropagation();openTerminal(${c.id})"') && remoteFrontend.includes("remoteDesktopProfilesForProfile") && remoteFrontend.includes("remoteDesktopSwitchProfiles") && remoteFrontend.includes('ondblclick="event.stopPropagation();${primary}"') && workspaceFrontend.includes("syncWorkspaceDocumentTitle") && workspaceFrontend.includes("Terma · ${endpoint} · ${label}"));
  ...
  ok("桌面窗口隐藏原生菜单栏且保留托盘右键菜单", desktopSource.includes('const PRODUCT_NAME = "Terma"') && appMenuSource.includes("Menu.setApplicationMenu(null)") && trayMenuSource.includes('label: `打开 ${PRODUCT_NAME}`') && trayMenuSource.includes('label: "在浏览器打开"') && trayMenuSource.includes('label: `退出 ${PRODUCT_NAME}`'));
  ...
  ok("桌面活动栏切换整个操作面板并支持调节宽度", !indexHtml.includes('id="operationPaneToggle"') && !indexHtml.includes('id="operationPaneExpand"') && indexHtml.includes('id="operationPaneCollapse"') && indexHtml.includes('id="activityBarResize"') && indexHtml.includes('class="brand-mark"') && indexHtml.includes('class="brand-name-full"') && indexHtml.includes("showPrimary('connections', true)") && indexHtml.includes("showPrimary('settings', true)") && workspaceFrontend.includes("name === primaryView ? !operationPaneCollapsed : false") && workspaceFrontend.includes('localStorage.setItem("operationPaneCollapsed"') && workspaceFrontend.includes('localStorage.setItem("activityBarWidth"') && appCss.includes("--activity-bar-width:40px") && appCss.includes("grid-template-columns:var(--activity-bar-width) minmax(0,1fr)") && appCss.includes(".brand-mark { width:24px") && appCss.includes(".brand-name-full { white-space:nowrap; }") && appCss.includes(".app.operation-pane-collapsed .brand-name-full"));
  ...
  ok("转发失败状态显示原因、时间并打开对应日期日志", workspaceFrontend.includes("存在启动失败的转发") && workspaceFrontend.includes("启动失败 ${failed}") && workspaceFrontend.includes("失败日志") && runningFrontend.includes("const failedCount") && runningFrontend.includes("forwardStatusText(forward.status)") && runningFrontend.includes("forward.last_error") && runningFrontend.includes("查看失败当天日志") && forwardsFrontend.includes("function forwardEventTimeText") && !forwardsFrontend.includes('|| "未运行"') && logsFrontend.includes("async function openSystemLogAt(timestamp)"));
  ok("停止全部转发会刷新状态并清理异常与待恢复项", forwardsFrontend.includes("function forwardNeedsStop(forward)") && forwardsFrontend.includes('status !== "stopped"') && forwardsFrontend.includes('Boolean(Number(forward?.restore || 0))') && forwardsFrontend.indexOf("await loadAll();") < forwardsFrontend.indexOf("const targets = connections.filter(c => (c.forwards || []).some(forwardNeedsStop));") && forwardsFrontend.includes("暂无需要停止或清理的转发") && desktopSource.includes("function forwardNeedsStop(forward)") && desktopSource.includes("(item.forwards || []).some(forwardNeedsStop)") && dbSource.includes('["failed", "reconnecting"].includes(forward.status)'));
  ok("连接转发操作使用紧凑图标并保留完整提示", appCss.includes(".conn-actions button { width:28px; min-width:28px;") && frontend.includes("connectionCompactToggleButton") && frontend.includes('aria-label="${text}"') && frontend.includes("connection-forward-toggle"));
  ok("Web 数据路径支持跨根目录浏览、安全远程管理与自动重启", settingsFrontend.includes("openStorageDirectoryBrowser") && settingsFrontend.includes("data-storage-root") && serverSource.includes('pathname === "/api/storage/directories"') && serverSource.includes("storageManagementAvailable") && serverSource.includes("!authRequired(req)") && serverSource.includes("restart-web.js") && fs.existsSync(path.join(root, "scripts/restart-web.js")) && read("src/config.ts").includes(".terma-storage.json") && read("src/config.ts").includes(".tunneldesk-storage.json"));
