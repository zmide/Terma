...
async function main() {
  ...
  ok("连接健康状态使用主题图标且保活字段保持紧凑", connectionsFrontend.includes('class="health-badge${healthClass}" title="健康状态：') && !connectionsFrontend.includes('${icon(health?.ok ? "circle-check" : health ? "circle-alert" : "circle-help")} ${esc(healthText)}') && indexHtml.includes("保活间隔（秒）") && indexHtml.includes('id="conn_keepalive_interval_help"'));
  ...
  ok("SFTP 支持跨主机流式复制", read("src/sftp-jobs.ts").includes("crossCopyJob") && read("src/sftp-jobs.ts").includes("source.stdout.pipe(target.stdin)") && sftpFrontend.includes("/sftp/cross-copy"));
  ...
  ok("新增 SSH 连接支持保存并清空", indexHtml.includes('id="connSaveAndClear"') && indexHtml.includes("保存并清空") && indexHtml.includes('data-action="static-connection-save-clear"') && frontend.includes("表单已清空") && frontend.includes('$("connSaveAndClear").hidden = true'));
  ok("SSH 测试失败保持在页面内且按钮测试期间禁用", indexHtml.includes('id="connTestBtn"') && indexHtml.includes('id="connTestStatus"') && indexHtml.includes('data-action="static-connection-test"') && frontend.includes('setButtonBusy(button, true, "测试中...")') && frontend.includes("SSH 测试无法完成") && read("src/ssh2-client.ts").includes("normalizeSshTransportError(error, connection)") && read("src/ssh2-client.ts").includes("SSH 握手超时，请检查主机地址、端口和 SSH 服务") && read("src/ssh2-client.ts").includes('client.on("error", onError)') && read("src/ssh2-client.ts").includes('client.on("error", reportError)') && read("src/ssh2-client.ts").includes('child.on("error", () => {})'));
