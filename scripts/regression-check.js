...
async function main() {
  ...
  ok("SSH 派生 RDP 直接按地址调用系统客户端", read("desktop/remote-clients.js").includes("source_ssh_connection_id") && read("desktop/remote-clients.js").includes('`/v:${profile.host}:${Number(profile.port || 3389)}`') && dbSource.includes('username:["vnc", "ftp"].includes(protocol)'));
