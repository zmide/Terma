...
async function main() {
  ...
  ok("第三方组件声明随桌面安装包提供", packageJson.build?.extraResources?.includes("THIRD_PARTY_NOTICES.md") && thirdPartySource.includes("listThirdPartyComponents") && ["xterm.js","Ace Editor","jsdiff","noVNC","ZMODEM.js","ssh2","i18next","Lucide","Electron","node-pty","VcXsrv"].every(name => thirdPartySource.includes(`name:\"${name}\"`)) && ["xterm.js","Ace Editor","jsdiff","noVNC","zmodem.js","ssh2","i18next","Lucide","Electron","node-pty","VcXsrv"].every(name => thirdPartyNotices.includes(`## ${name}`)));
  ...
  ok("全局终端设置独立持久化并应用到当前和未来会话",
    runtimeSettingsSource.includes("DEFAULT_TERMINAL_SETTINGS")
      ...
      && runtimeSettingsSource.includes("schema_version: 14")
  ...
  ok("其他协议编辑页的关闭按钮会关闭当前工作区标签", remoteFrontend.includes('onclick="closeTabsByKey([activeTabKey],activeTabKey)">${esc(tr("common:actions.close", {defaultValue:"关闭"}))}</button>') && !remoteFrontend.includes('renderConnections();backToExplorer()">取消</button>'));
  ...
  ok(
    ...
    Array.isArray(about?.third_party_components)
      && about.third_party_components.length >= 10
