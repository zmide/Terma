...
async function main() {
  ...
  ok("SFTP 多文件传输提前进入统一队列并支持批量控制", read("public/index.html").includes('id="sftpTaskCenterPauseAllButton"') && read("public/index.html").includes('id="sftpTaskCenterResumeAllButton"') && read("public/index.html").includes('id="sftpTaskCenterClearQueueButton"') && sftpFrontend.includes("function sftpTransferQueueJobs") && sftpFrontend.includes("function pauseAllSftpTransferJobs") && sftpFrontend.includes("function resumeAllSftpTransferJobs") && sftpFrontend.includes("function clearCurrentSftpTransferQueue") && sftpFrontend.includes("const uploads = []") && sftpFrontend.includes("uploads.push({item, started})") && sftpTransferRoutesSource.includes("paths.map((remotePath: string) => dependencies.startLocalDeliveryJob") && sftpTransferRoutesSource.includes('{jobs, count:jobs.length, status:"pending"}') && sftpJobsSource.includes("job.abortController?.abort?.()") && sftpSessionSource.includes("signal:progress?.signal"));
  ok("SFTP 生成文件使用安全中文文件名并提供打开、删除入口", read("public/app-sftp-generated-tasks.js").includes('encodeURIComponent(String(filename') && read("public/app-sftp-generated-tasks.js").includes("deleteSftpGeneratedTaskFile") && read("src/routes/sftp-desktop-download-routes.ts").includes("decodeGeneratedFilename") && read("src/routes/sftp-desktop-download-routes.ts").includes('contentType.startsWith("image/svg+xml")') && read("src/routes/sftp-desktop-download-routes.ts").includes('pathname === "/api/sftp/download-settings/delete-generated"') && sftpFrontend.includes('job.type === "local-generated" && job.delivery_status === "saved"') && sftpFrontend.includes("openSftpGeneratedTaskDirectory"));
  ...
  ok(
    ...
    read("public/index.html").includes('id="toast" class="notification-stack"')
      ...
      && sftpCss.includes("--z-notification:60100")
      && sftpCss.includes("z-index:var(--z-notification)")
  ...
  ok("SFTP 文本查找与 SVG 源码定位支持单行横向滚动并强化选区高亮", read("public/app-sftp-svg-source.js").includes("findSourceRange") && read("public/app-sftp-svg-source.js").includes("selection.setRange") && read("public/app-sftp-svg-source.js").includes("scrollCursorIntoView") && read("public/app-sftp-svg-source.js").includes("sftp-svg-source-target") && read("public/app-sftp-editor.js").includes("renderer?.scrollCursorIntoView") && read("public/app-sftp-editor.js").includes("fallbackEditor.scrollLeft") && read("public/app.css").includes(".ace_selection") && read("public/app.css").includes(".sftp-svg-source-target") && read("scripts/ui-smoke-electron.js").includes("svgSplitOneLineSourceFocus"));
  ok(
  ...
  const narrowWorkspaceCss = appCss.slice(
  ...
  ok(
    "SSH 连接卡移除易误解的转发运行标识且仅显示已检测健康状态",
    connectionsFrontend.includes('const healthBadge = health ?')
      && connectionsFrontend.includes('<div class="conn-main">')
      && !connectionsFrontend.includes('class="conn-state"')
      && !connectionsFrontend.includes('class="status-dot')
      && !connectionsFrontend.includes('icon(health?.ok ? "circle-check" : health ? "circle-alert" : "circle-help")')
  );
  ...
  ok("运行中更新检查只刷新红点且不弹通知", notificationsSource.includes("event.silent === true ? {silent:true}") && notificationsSource.includes("if (!item.silent) appendNotificationSystemLog") && frontend.includes("event.silent !== true") && desktopSource.includes("event?.silent !== true"));
  ...
  ok("转发列表支持分组、服务器折叠和持久排序", frontend.includes("globalForwardGroupedConnections") && frontend.includes("global-forward-group-toggle") && frontend.includes("global-forward-connection-toggle") && frontend.includes("beginGlobalForwardSortDrag") && frontend.includes("/api/connections/reorder") && serverSource.includes("reorderConnections") && dbSource.includes("function reorderConnections"));
  ok("SSH 连接分组手动收起后不被后台刷新重新展开", !appEntry.includes("if (selectedId === c.id) groupOpen.add(c.group_name)") && frontend.includes("function selectConnection(id, options={})") && frontend.includes("groupOpen.add(c.group_name)") && frontend.includes("function toggleGroupOpen(group)"));
  ...
  ok("通用设置瀑布布局按分屏容器宽度自适应", narrowWorkspaceCss.includes(".settings-grid.settings-waterfall { column-count:1; }"));
  ...
  ok("远程桌面支持 TigerVNC、手动 noVNC 与系统客户端回退", Boolean(packageJson.dependencies?.["@novnc/novnc"]) && packageJson.scripts?.["vnc:prepare"] && serverSource.includes('pathname.startsWith("/vendor/novnc/")') && remoteFrontend.includes('client_mode:$("remote_vnc_client_mode").value') && remoteFrontend.includes('value="bundled"') && remoteFrontend.includes("bundled_tigervnc") && dbSource.includes('client_mode:new Set(["auto", "bundled", "embedded", "system"])') && read("src/vnc-proxy.ts").includes("handleVncUpgrade"));
  ok("VNC 支持可选保存密码并在认证失败后重输更新", remoteFrontend.includes("handleVncSecurityFailure") && remoteFrontend.includes("update_saved_password") && remoteFrontend.includes("自动传给内置 TigerVNC Viewer") && serverSource.includes('parts[3] === "vnc-credential"') && dbSource.includes("getVncProfileCredential") && fs.existsSync(path.join(root, "scripts/vnc-credential-check.js")));
  ...
  ok("VNC 新窗口使用原生标题栏并保持弹窗通知可见", desktopSource.includes('fetchJson("/api/remote-profiles")') && !desktopSource.includes("terma:vnc-window-maximize") && !read("desktop/preload.js").includes("toggleVncWindowMaximize") && !remoteFrontend.includes("data-vnc-window-maximize") && appCss.includes("html.vnc-fullscreen-document:fullscreen .modal { z-index:60030; }") && appCss.includes("html.vnc-fullscreen-document:fullscreen .notification-stack { z-index:var(--z-notification); }") && appCss.includes("body.vnc-detached-window-body .app { display:none !important; }") && !appCss.includes("vnc-detached-window-body #toast") && !appCss.includes("vnc-detached-window-body #modal"));
  ...
  ok("连接健康状态使用主题图标且保活字段保持紧凑", connectionsFrontend.includes('class="health-badge${healthClass}"') && connectionsFrontend.includes('title="${escAttr(healthStatusText)}"') && connectionsFrontend.includes('connections:groups.health_status') && connectionsFrontend.includes('icon(health.ok ? "circle-check" : "circle-alert")') && indexHtml.includes("保活间隔（秒）") && indexHtml.includes('id="conn_keepalive_interval_help"'));
  const commandPaletteSource = read("public/app-command-palette.js");
  ok("命令窗口默认不铺开连接和命令片段并在搜索时显示远程资源", connectionsFrontend.includes("function quickForwardRows") && connectionsFrontend.includes("quickForwardSearchText") && connectionsFrontend.includes("QUICK_FORWARD_RESULT_LIMIT = 120") && connectionsFrontend.includes("requestAnimationFrame") && connectionsFrontend.includes('data-dblclick-action="quick-connect-forward"') && connectionsFrontend.includes('data-forward-id="${forward.id}"') && connectionsFrontend.includes("url_path") && commandPaletteSource.includes("quick-panel-remote-open") && commandPaletteSource.includes("registerTermaAction") && commandPaletteSource.includes("hasConnectionSearch") && commandPaletteSource.includes("scheduleQuickPanelRender") && commandPaletteSource.includes("navigation:auto.command_window") && commandPaletteSource.includes('if (hasConnectionSearch)') && commandPaletteSource.includes('kind:"snippet"'));
  ok("SFTP Shell 换行符可选且非 LF 保存有明确兼容性确认", sftpFrontend.includes("unsafeScriptLineEnding") && sftpFrontend.includes("shell_line_ending_warning") && sftpFrontend.includes('id="sftpLineEnding"') && !sftpFrontend.includes('id="sftpLineEnding" ${unixScript ? "disabled"') && read("src/services/sftp-content-service.ts").includes('["lf", "crlf", "cr"].includes(requestedLineEnding) ? requestedLineEnding : (unixScript ? "lf" : "")'));
  ok("SFTP 编辑统计与全文搜索合并连续输入并在保存时精确校验", sftpFrontend.includes("scheduleEditorStats") && sftpFrontend.includes("editorSearchTimer") && sftpFrontend.includes("}, 120)") && sftpFrontend.includes("updateStats(true, prepared.content, prepared.encoding)"));
  ...
  ok("SFTP 任务中心宽高跨重启持久化且异常尺寸可恢复", sftpTasksFrontend.includes('SFTP_TASK_CENTER_SIZE_STORAGE_KEY = "sftpTaskCenterSizeV1"') && sftpTasksFrontend.includes("persistSftpTaskCenterSize") && sftpTasksFrontend.includes("restoreSftpTaskCenterSize") && sftpTasksFrontend.includes('drawer.closest(".content")') && sftpTasksFrontend.includes("ensureSftpTaskCenterBoundsMonitoring") && sftpTasksFrontend.includes("localStorage.removeItem(SFTP_TASK_CENTER_SIZE_STORAGE_KEY)") && indexHtml.includes('id="sftpTaskCenterResetSizeButton"'));
  ...
  ok("SFTP 同一连接同一路径的文本编辑器复用并可恢复最小化窗口", sftpFrontend.includes("sftpTextEditorOpenKey") && sftpFrontend.includes("activateSftpTextEditor") && sftpFrontend.includes("sftpFloatingEditorRegistry") && sftpFrontend.includes("editorKey"));
  ok("SFTP 后台目录读取不因切换标签丢失并在恢复时重绘", /const data = await requestSftpDirectoryPage\([\s\S]{0,320}if \(!sftpTabRuntimes\.has\(tabKey\) \|\| requestSeq !== runtime\.state\.requestSeq\) return false;\s*const mounted/.test(sftpFrontend) && sftpFrontend.includes("if (!runtime.state.loading && Array.isArray(runtime.state.entries))") && sftpFrontend.includes("requestSftpDirectoryPage") && !sftpFrontend.includes("if (!isSftpOpenActivationCurrent({...options, tabKey})) return false;\n    const mounted"));
  ...
  ok("终端拖入继续使用 SFTP 并明确原登录用户权限边界", terminalFrontend.includes("uploadSftpFilesToDirectory(files, connection.id, directory)") && terminalFrontend.includes("terminal:drop.sftp_login_identity_hint") && terminalFrontend.includes("terminal:drop.upload_failed_identity_hint") && !terminalFrontend.includes("terminalZmodemUploadDroppedFiles"));
  ...
  ok("SFTP 大文件任务使用单一防抖写入且桌面保存不阻塞主循环", sftpJobsSource.includes("persistRequestedAt") && sftpJobsSource.includes("const flush = () =>") && sftpJobsSource.includes('require("node:stream/promises")') && sftpJobsSource.includes("await pipeline(input, output, {signal})") && sftpJobsSource.includes('job.phase = "system-saving"') && !sftpJobsSource.includes("copyFileSync") && read("src/sftp-job-store.ts").includes("fs.renameSync(temporary, file)") && read("src/logs.ts").includes("queueLogWrite") && !read("src/logs.ts").includes("fs.appendFileSync(logFile"));
  ...
  try {
    ...
    const windowResult = systemLog
      ...
      : null;
    const searchResult = await fetch(`${base}/api/logs/search?query=${encodeURIComponent("Terma")}`).then(response=>response.json());
    ok("日志设置、分段读取、内容搜索和服务端搜索接口", savedLogSettings.retention_days === 7 && savedLogSettings.max_file_size_mb === 1 && savedLogSettings.max_total_size_mb === 10 && savedLogSettings.rotation_files === 2 && windowResult && typeof windowResult.text === "string" && Number.isInteger(windowResult.offset) && Array.isArray(windowResult.matches) && Array.isArray(searchResult) && searchResult.every(item => item.path && Array.isArray(item.matches)));
  } catch (error) {
