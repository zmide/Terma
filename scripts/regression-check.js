...
async function main() {
  ...
  ok("远程桌面支持 TigerVNC、noVNC 与系统客户端三级回退", Boolean(packageJson.dependencies?.["@novnc/novnc"]) && packageJson.scripts?.["vnc:prepare"] && serverSource.includes('pathname.startsWith("/vendor/novnc/")') && remoteFrontend.includes('client_mode:$("remote_vnc_client_mode").value') && remoteFrontend.includes('value="bundled"') && remoteFrontend.includes("bundled_tigervnc") && dbSource.includes('client_mode:new Set(["auto", "bundled", "embedded", "system"])') && read("src/vnc-proxy.ts").includes("handleVncUpgrade"));
