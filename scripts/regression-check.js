...
async function main() {
  ...
  ok("SFTP 命令不依赖远端登录 Shell", sftpBackend.includes("spawnSftpSessionCommand(connection, buildRemotePosixCommand(command))") && read("src/sftp-jobs.ts").includes("spawnSftpSessionCommand(connection, buildRemotePosixCommand(command))"));
  ...
  ok("全局终端设置独立持久化并应用到当前和未来会话",
    runtimeSettingsSource.includes("DEFAULT_TERMINAL_SETTINGS")
      ...
      && runtimeSettingsSource.includes("schema_version: 9")
      ...
      && terminalFrontend.includes("应用到继承默认值的连接；连接独立设置优先")
  );
