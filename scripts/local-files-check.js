...
assert.match(sftpTasksUi, /queueSftpDirectoryRefresh\(connectionId\)[\s\S]*?flushPendingSftpDirectoryRefresh\(\)/, "SFTP completion notifications must queue directory refresh before the authoritative poll");
assert.match(sftpTasksUi, /function sftpTaskCenterRenderSignature\(/, "task center polling must compare a lightweight signature before rendering job markup");
assert.match(sftpTasksUi, /if \(list\._sftpTaskCenterRenderSignature !== renderSignature\) \{[\s\S]*?visible\.map\(renderSftpJob\)/, "task markup must only be generated after its lightweight signature changes");
assert.match(sftpUi, /const SFTP_DIRECTORY_REQUEST_CONCURRENCY = 3/, "rapid SFTP opens must use a bounded directory request concurrency");
assert.match(sftpUi, /function drainSftpDirectoryRequestQueue\(/);
assert.match(sftpUi, /scheduleSftpDirectoryRequest\(\(\) => api\(requestPath\)\)/, "shared directory requests must enter the bounded queue before per-tab cancellation");
