...
async function main() {
  ...
  const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "tunneldesk-packaged-backend-"));
  process.env.TUNNELDESK_DATA_DIR = path.join(temporary, "data");
  process.env.TUNNELDESK_SSH_DIR = path.join(temporary, "ssh");
  ...
  try {
    ...
    if (!response.ok || about.product_name !== "TunnelDesk" || about.version !== metadata.version) {
    ...
    if (process.platform === "darwin") {
      const addon = require(path.join(`${archive}.unpacked`, "native", "macos-sftp-drag", "prebuilds", `darwin-${process.arch}`, "tunneldesk_macos_sftp_drag.node"));
