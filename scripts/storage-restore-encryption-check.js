...
function encrypted(value) {
  return typeof value === "string" && /^(?:tdenc:v1|termaenc:v[123]):/.test(value);
}
...
try {
  ...
  `).run(
    ...
    cryptoStore.encryptText("key-passphrase"), "-o Compression=yes", "/bin/bash", "-lc", "/tmp"
  );
  database.prepare(`
    INSERT INTO connections(
      id,name,ssh_host,ssh_port,ssh_user,auth_type,identity_file,ssh_password,
  ...
  assert.match(keyRow.private_key_passphrase, /^termaenc:v3:/);
  assert.match(passwordRow.ssh_password, /^termaenc:v3:/);
  ...
  assert.match(afterValidation, /^termaenc:v3:/);
  ...
  console.log("配置加密恢复回归通过：未解锁拒绝恢复，解锁后原始 SQLite 敏感字段全部使用 termaenc:v3");
} finally {
