...
kill_by_name() {
  count=0
  for pattern in "$ROOT_DIR/dist/server.js" "$ROOT_DIR/desktop/main.js" "node dist/server.js" "electron ." "Terma" "TunnelDesk" "tunneldesk"; do
...
if command -v node >/dev/null 2>&1; then
  if node -e "fetch(process.argv[1] + '/api/shutdown', { method: 'POST' }).then(r => process.exit(r.ok ? 0 : 1)).catch(() => process.exit(1))" "$URL"; then
    echo "Stopped Terma gracefully, pid=$PID"
    kill_by_name
    exit 0
  fi
fi
